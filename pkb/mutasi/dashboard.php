...
// --- PROSES DATA UNTUK MUTASI MASUK ---
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_masuk'])) {
    ...
    $sql = "INSERT INTO mutasi_masuk (nomor_kendaraan, nomor_uji, nama_pemilik, tujuan, nomor_surat, link_file_gdrive) VALUES (?, ?, ?, ?, ?, ?)";
    if ($stmt = $koneksi->prepare($sql)) {
        $stmt->bind_param("ssssss", $nomor_kendaraan, $nomor_uji, $nama_pemilik, $tujuan, $nomor_surat, $link_file_gdrive);
        if ($stmt->execute()) {
            echo "<script>alert('Data Mutasi Masuk berhasil disimpan!'); window.location.href='dashboard.php';</script>";
        } else {
...
if (isset($_GET['hapus_masuk_id'])) {
    ...
    $sql_hapus = "DELETE FROM mutasi_masuk WHERE id = ?";
    if ($stmt = $koneksi->prepare($sql_hapus)) {
        $stmt->bind_param("i", $hapus_id);
        if ($stmt->execute()) {
            echo "<script>alert('Data Mutasi Masuk berhasil dihapus!'); window.location.href='dashboard.php';</script>";
        } else {
...
// --- PROSES DATA UNTUK MUTASI KELUAR ---
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_keluar'])) {
    ...
    $sql = "INSERT INTO mutasi_keluar (nomor_kendaraan, nomor_uji, nama_pemilik, tujuan, nomor_surat, link_file_gdrive) VALUES (?, ?, ?, ?, ?, ?)";
    if ($stmt = $koneksi->prepare($sql)) {
        $stmt->bind_param("ssssss", $nomor_kendaraan, $nomor_uji, $nama_pemilik, $tujuan, $nomor_surat, $link_file_gdrive);
        if ($stmt->execute()) {
            echo "<script>alert('Data Mutasi Keluar berhasil disimpan!'); window.location.href='dashboard.php#keluar-tab-pane';</script>";
        } else {
            echo "<script>alert('Terjadi kesalahan: " . $stmt->error . "');</script>";
        }
        $stmt->close();
...
if (isset($_GET['hapus_keluar_id'])) {
    ...
    $sql_hapus = "DELETE FROM mutasi_keluar WHERE id = ?";
    if ($stmt = $koneksi->prepare($sql_hapus)) {
        $stmt->bind_param("i", $hapus_id);
        if ($stmt->execute()) {
            echo "<script>alert('Data Mutasi Keluar berhasil dihapus!'); window.location.href='dashboard.php#keluar-tab-pane';</script>";
        } else {
            echo "<script>alert('Terjadi kesalahan saat menghapus data.');</script>";
        }
        $stmt->close();
...
$search_query_masuk = "";
$search_param_masuk = [];
$sql_search_masuk = "SELECT * FROM mutasi_masuk";
$sql_count_masuk = "SELECT COUNT(*) AS total FROM mutasi_masuk";
...
$search_query_keluar = "";
$search_param_keluar = [];
$sql_search_keluar = "SELECT * FROM mutasi_keluar";
$sql_count_keluar = "SELECT COUNT(*) AS total FROM mutasi_keluar";
...
<head>
    ...
    <title>Dashboard Mutasi - UPT PKB Gresik</title>
...
<body>
    ...
    <main class="main">
            ...
            <div class="heading">
                <div class="container">
                    <div class="row d-flex justify-content-center text-center">
                        <div class="col-lg-8">
                            <h1>Dashboard Mutasi</h1>
                            <p class="mb-0">Kelola data rekomendasi mutasi masuk dan keluar.</p>
                        </div>
                    </div>
                </div>
            </div>
            <nav class="breadcrumbs">
                <div class="container">
                    <ol>
                        ...
                        <li class="current">Dashboard Mutasi</li>
                    </ol>
                </div>
            </nav>
        </div><!-- End Page Title -->
        ...
        <div class="container mt-5">
            ...
            <ul class="nav nav-tabs" id="mutasiTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="masuk-tab" data-bs-toggle="tab" data-bs-target="#masuk-tab-pane" type="button" role="tab" aria-controls="masuk-tab-pane" aria-selected="true">Mutasi Masuk</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="keluar-tab" data-bs-toggle="tab" data-bs-target="#keluar-tab-pane" type="button" role="tab" aria-controls="keluar-tab-pane" aria-selected="false">Mutasi Keluar</button>
                </li>
            </ul>

            <div class="tab-content" id="mutasiTabContent">
                <!-- ================================== -->
                <!-- TAB CONTENT UNTUK MUTASI MASUK -->
                <!-- ================================== -->
                <div class="tab-pane fade show active" id="masuk-tab-pane" role="tabpanel" aria-labelledby="masuk-tab" tabindex="0">
                    <div class="py-4">
                        <h2>Form Mutasi Masuk</h2>
                            ...
                            <tbody>
                                ...
                                if ($result_search_masuk->num_rows > 0) {
                                    ...
                                    while ($row = $result_search_masuk->fetch_assoc()) {
                                        echo "<tr><td>" . $no++ . "</td><td>" . htmlspecialchars($row['nomor_kendaraan']) . "</td><td>" . htmlspecialchars($row['nomor_uji']) . "</td><td>" . htmlspecialchars($row['nama_pemilik']) . "</td><td>" . htmlspecialchars($row['tujuan']) . "</td><td>" . htmlspecialchars($row['nomor_surat']) . "</td><td><a href='" . htmlspecialchars($row['link_file_gdrive']) . "' target='_blank'>Lihat</a></td><td><a href='edit_mutasi_masuk.php?id=" . $row['id'] . "' class='btn btn-warning btn-sm'>Edit</a> <a href='dashboard.php?hapus_masuk_id=" . $row['id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data ini?\")'>Hapus</a></td></tr>";
                                    }
                            ...
                            <tbody>
                                ...
                                if ($result_search_keluar->num_rows > 0) {
                                    ...
                                    while ($row = $result_search_keluar->fetch_assoc()) {
                                        echo "<tr><td>" . $no++ . "</td><td>" . htmlspecialchars($row['nomor_kendaraan']) . "</td><td>" . htmlspecialchars($row['nomor_uji']) . "</td><td>" . htmlspecialchars($row['nama_pemilik']) . "</td><td>" . htmlspecialchars($row['tujuan']) . "</td><td>" . htmlspecialchars($row['nomor_surat']) . "</td><td><a href='" . htmlspecialchars($row['link_file_gdrive']) . "' target='_blank'>Lihat</a></td><td><a href='edit_mutasi_keluar.php?id=" . $row['id'] . "' class='btn btn-warning btn-sm'>Edit</a> <a href='dashboard.php?hapus_keluar_id=" . $row['id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data ini?\")'>Hapus</a></td></tr>";
                                    }
                                } else { echo "<tr><td colspan='8' class='text-center'>Tidak ada data.</td></tr>"; }
                                ?>
                            </tbody>
