...
// Proses untuk menyimpan data mutasi keluar (hanya untuk yang sudah login)
if ($form_visible && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    // Query untuk menyimpan data ke tabel mutasi_keluar
    $sql = "INSERT INTO mutasi_keluar (nomor_kendaraan, nomor_uji, nama_pemilik, tujuan, nomor_surat, link_file_gdrive)
...
if (isset($_GET['hapus_id'])) {
    ...
    $sql_hapus = "DELETE FROM mutasi_keluar WHERE id = '$hapus_id'";
    if ($koneksi->query($sql_hapus) === TRUE) {
        echo "<script>alert('Data berhasil dihapus!');</script>";
        echo "<script>window.location.href = 'mutasi_keluar.php';</script>";
    } else {
...
<head>
    ...
    <script>
        function toUpperCaseInput(event) {
            event.target.value = event.target.value.toUpperCase().replace(/\s/g, ''); // Menghapus spasi
        }

        function toUpperCaseSearch(event) {
            event.target.value = event.target.value.toUpperCase().replace(/\s/g, ''); // Menghapus spasi pada pencarian
        }
    </script>
</head>
    ...
    <main class="main">
        <div class="container mt-5">
            ...
            <?php if ($form_visible): ?>
                ...
                <form action="mutasi_keluar.php" method="POST">
                    <div class="mb-3">
                ...
                <tbody>
                    ...
                    if ($result_search->num_rows > 0) {
                        ...
                        while ($row = $result_search->fetch_assoc()) {
                            ...
                            // Menampilkan tombol Edit dan Hapus jika sudah login
                            if (!$hide_actions) {
                                echo "<td>
                                        <a href='edit_mutasi_keluar.php?id=" . $row['id'] . "' class='btn btn-warning btn-sm'>Edit</a>
                                        <a href='mutasi_keluar.php?hapus_id=" . $row['id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data?\")'>Hapus</a>
                                      </td>";
                            }
                        ...
                        echo "<tr><td colspan='8' class='text-center'>Tidak ada data mutasi keluar.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
            ...
            <div class="mt-4">
                <ul class="pagination">
                    ...
                    for ($i = 1; $i <= $total_pages; $i++) {
                        echo "<li class='page-item " . ($i == $page ? 'active' : '') . "'><a class='page-link' href='mutasi_keluar.php?page=$i'>$i</a></li>";
                    }
                    ?>
                </ul>
            </div>
