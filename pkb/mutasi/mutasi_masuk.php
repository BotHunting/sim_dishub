...
// Proses untuk menyimpan data mutasi masuk
if ($form_visible && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $link_file_gdrive = $_POST['link_file_gdrive'];

    $sql = "INSERT INTO mutasi_masuk (nomor_kendaraan, nomor_uji, nama_pemilik, tujuan, nomor_surat, link_file_gdrive)
...
if (isset($_GET['hapus_id'])) {
    ...
    $sql_hapus = "DELETE FROM mutasi_masuk WHERE id = '$hapus_id'";
    if ($koneksi->query($sql_hapus) === TRUE) {
        echo "<script>alert('Data berhasil dihapus!');</script>";
        echo "<script>window.location.href = 'mutasi_masuk.php';</script>";
    } else {
    ...
    <main class="main">
        <div class="container mt-5">
            ...
            <?php if ($form_visible): ?>
                ...
                <form action="mutasi_masuk.php" method="POST">
                    <div class="mb-3">
                ...
                <tbody>
                    ...
                    if ($result_search->num_rows > 0) {
                        ...
                        while ($row = $result_search->fetch_assoc()) {
                            ...
                            if (!$hide_actions) {
                                echo "<td>
                                        <a href='edit_mutasi_masuk.php?id=" . $row['id'] . "' class='btn btn-warning btn-sm'>Edit</a>
                                        <a href='mutasi_masuk.php?hapus_id=" . $row['id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data?\")'>Hapus</a>
                                      </td>";
                            }
                        ...
                        echo "<tr><td colspan='8' class='text-center'>Tidak ada data mutasi masuk.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
            ...
            <div class="mt-4">
                <ul class="pagination">
                    ...
                    for ($i = 1; $i <= $total_pages; $i++) {
                        echo "<li class='page-item " . ($i == $page ? 'active' : '') . "'><a class='page-link' href='mutasi_masuk.php?page=$i'>$i</a></li>";
                    }
                    ?>
                </ul>
            </div>
