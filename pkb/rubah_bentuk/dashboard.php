...
// Proses untuk menyimpan data Rubah Bentuk (hanya untuk yang sudah login)
if ($form_visible && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $rubah_bentuk_ke = strtoupper($_POST['rubah_bentuk_ke']);
    $nomor_surat = strtoupper($_POST['nomor_surat']);
    $link_file_gdrive = $_POST['link_file_gdrive'];  // Link tetap dalam format asli

    // Query untuk menyimpan data ke tabel rubah_bentuk
    $sql = "INSERT INTO rubah_bentuk (nomor_kendaraan, nomor_uji, nama_pemilik, rubah_bentuk_ke, nomor_surat, link_file_gdrive)
            VALUES ('$nomor_kendaraan', '$nomor_uji', '$nama_pemilik', '$rubah_bentuk_ke', '$nomor_surat', '$link_file_gdrive')";
...
$sql_count = "SELECT COUNT(*) AS total FROM rubah_bentuk";
...
if (isset($_GET['hapus_id'])) {
    ...
    $sql_hapus = "DELETE FROM rubah_bentuk WHERE id = '$hapus_id'";
...
<head>
    ...
    <title>Dashboard Rubah Bentuk - UPT PKB Gresik</title>
        ...
        <div class="container mt-5">
            ...
            <?php if ($form_visible): ?>
                ...
                <form action="dashboard.php" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="rubah_bentuk_ke" class="form-label">Rubah Bentuk Ke</label>
                        <input type="text" class="form-control" id="rubah_bentuk_ke" name="rubah_bentuk_ke" required oninput="toUpperCaseInput(event)">
                    </div>
                    <div class="mb-3">
                ...
                <tbody>
                    ...
                    if ($result_search->num_rows > 0) {
                        ...
                        while ($row = $result_search->fetch_assoc()) {
                            ...
                            if (!$hide_actions) {
                                echo "<td>
                                        <a href='edit_rubah_bentuk.php?id=" . $row['id'] . "' class='btn btn-warning btn-sm'>Edit</a>
                        ...
                        echo "<tr><td colspan='8' class='text-center'>Tidak ada data rubah bentuk.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
