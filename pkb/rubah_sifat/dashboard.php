...
// Proses untuk menyimpan data rubah sifat (hanya untuk yang sudah login)
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $rubah_sifat = strtoupper($_POST['rubah_sifat']);
    $nomor_surat = strtoupper($_POST['nomor_surat']);
    $link_file_gdrive = $_POST['link_file_gdrive']; // Link tetap dalam format asli

    // Query untuk menyimpan data ke tabel rubah_sifat
    $sql = "INSERT INTO rubah_sifat (nomor_kendaraan, nomor_uji, nama_pemilik, rubah_sifat, nomor_surat, link_file_gdrive)
            VALUES ('$nomor_kendaraan', '$nomor_uji', '$nama_pemilik', '$rubah_sifat', '$nomor_surat', '$link_file_gdrive')";
...
if (isset($_POST['search'])) {
    ...
    $sql_search = "SELECT * FROM rubah_sifat WHERE nomor_surat LIKE '%$search_query%' ORDER BY created_at DESC";
} else {
    $sql_search = "SELECT * FROM rubah_sifat ORDER BY created_at DESC";
}
...
$sql_count = "SELECT COUNT(*) AS total FROM rubah_sifat";
...
if (isset($_GET['hapus_id'])) {
    ...
    $sql_hapus = "DELETE FROM rubah_sifat WHERE id = '$hapus_id'";
...
<head>
    ...
    <title>Dashboard Rubah Sifat - UPT PKB Gresik</title>
...
<body>
    ...
    <main class="main">
        <div class="container mt-5">
            ...
            <h2>Form Rubah Sifat Kendaraan</h2>
            <form action="dashboard.php" method="POST">
                ...
                <div class="mb-3">
                    <label for="rubah_sifat" class="form-label">Rubah Sifat Ke</label>
                    <input type="text" class="form-control" id="rubah_sifat" name="rubah_sifat" required oninput="toUpperCaseInput(event)">
                </div>
                <div class="mb-3">
            ...
            <h3 class="mt-5">Tabel Rubah Sifat</h3>
                ...
                <tbody>
                    ...
                    if ($result_search->num_rows > 0) {
                        ...
                        while ($row = $result_search->fetch_assoc()) {
                            ...
                            echo "<td>" . $row['rubah_sifat'] . "</td>";
                            echo "<td>" . $row['nomor_surat'] . "</td>";
                            echo "<td><a href='" . $row['link_file_gdrive'] . "' target='_blank'>Lihat</a></td>";
                            echo "<td>
                                    <a href='edit_rubah_sifat.php?id=" . $row['id'] . "' class='btn btn-warning btn-sm'>Edit</a>
                                    <a href='dashboard.php?hapus_id=" . $row['id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data?\")'>Hapus</a>
                                  </td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='8' class='text-center'>Tidak ada data rubah sifat.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
            ...
            <div class="mt-4">
                <ul class="pagination">
                    ...
                    for ($i = 1; $i <= $total_pages; $i++) {
                        echo "<li class='page-item " . ($i == $page ? 'active' : '') . "'><a class='page-link' href='dashboard.php?page=$i'>$i</a></li>";
                    }
                    ?>
                </ul>
            </div>
