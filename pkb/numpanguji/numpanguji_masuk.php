...
// Proses untuk menyimpan data Numpang Uji Masuk (hanya untuk yang sudah login)
if ($form_visible && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    // Query untuk menyimpan data ke tabel numpanguji_masuk
    $sql = "INSERT INTO numpanguji_masuk (nomor_kendaraan, nomor_uji, nama_pemilik, tujuan, nomor_surat, link_file_gdrive)
...
if (isset($_POST['search'])) {
    ...
    $sql_search = "SELECT * FROM numpanguji_masuk WHERE nomor_surat LIKE '%$search_query%' ORDER BY created_at DESC";
} else {
    $sql_search = "SELECT * FROM numpanguji_masuk ORDER BY created_at DESC";
}
...
$sql_count = "SELECT COUNT(*) AS total FROM numpanguji_masuk";
...
if (isset($_GET['hapus_id'])) {
    ...
    $sql_hapus = "DELETE FROM numpanguji_masuk WHERE id = '$hapus_id'";
    if ($koneksi->query($sql_hapus) === TRUE) {
        echo "<script>alert('Data berhasil dihapus!');</script>";
        echo "<script>window.location.href = 'numpanguji_masuk.php';</script>";
    } else {
...
<head>
    ...
    <title>Numpang Uji Masuk - UPT PKB Gresik</title>
    ...
    <main class="main">
        <div class="container mt-5">
            ...
            <?php if ($form_visible): ?>
                ...
                <form action="numpanguji_masuk.php" method="POST">
                    <div class="mb-3">
                ...
                <tbody>
                    ...
                    if ($result_search->num_rows > 0) {
                        ...
                        while ($row = $result_search->fetch_assoc()) {
                            ...
                            if (!$hide_actions) {
                                echo "<td>
                                        <a href='edit_numpanguji_masuk.php?id=" . $row['id'] . "' class='btn btn-warning btn-sm'>Edit</a>
                                        <a href='numpanguji_masuk.php?hapus_id=" . $row['id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data?\")'>Hapus</a>
                                      </td>";
                            }
                        ...
                        echo "<tr><td colspan='8' class='text-center'>Tidak ada data numpang uji masuk.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
            ...
            <div class="mt-4">
                <ul class="pagination">
                    ...
                    for ($i = 1; $i <= $total_pages; $i++) {
                        echo "<li class='page-item " . ($i == $page ? 'active' : '') . "'><a class='page-link' href='numpanguji_masuk.php?page=$i'>$i</a></li>";
                    }
                    ?>
                </ul>
            </div>
