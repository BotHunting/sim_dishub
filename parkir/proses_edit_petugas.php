    ...
    if (isset($_POST['id'])) {
        ...
        if ($_FILES['foto']['error'] == UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $foto_path)) {
                ...
                $sql = "UPDATE petugas_parkir SET nama=?, nip=?, jabatan=?, jadwal_kerja=?, telepon=?, foto=? WHERE id=?";
            ...
            $sql = "UPDATE petugas_parkir SET nama=?, nip=?, jabatan=?, jadwal_kerja=?, telepon=? WHERE id=?";
            $stmt = $koneksi->prepare($sql);
