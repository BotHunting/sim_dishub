...
// Mendapatkan data laporan untuk bulan yang dipilih
$query_laporan = "SELECT * FROM laporan WHERE MONTH(tanggal) = $current_month AND status = 'Approved' ORDER BY tanggal DESC";
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_assoc($result_laporan)) : ?>
                    <tr>
                        ...
                        <td>
                            <a href="edit_laporan.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-primary">Edit</a>
                            <a href="hapus_laporan.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-danger">Hapus</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
