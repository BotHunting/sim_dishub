...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username_err) && empty($password_err)) {
        $sql = "SELECT id, username, password, rules FROM admin WHERE username = ?";
        if ($stmt = $koneksi->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                $stmt->store_result();
                if ($stmt->num_rows == 1) {
                    $stmt->bind_result($id, $username, $stored_password, $rules);
                    if ($stmt->fetch()) {
                        if ($password == $stored_password) {
                            ...
                            $_SESSION["id"] = $id; // Menyimpan ID pengguna ke dalam session
                            header("location: index.php");
                        } else {
